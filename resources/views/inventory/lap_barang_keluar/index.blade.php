...
@section('judul',"Outgoing Items Report")
...
<div class="col-md-9" style="margin:0 auto;">
        ...
        <div class="card-header card-header-warning">
            <h4 class="card-title mt-0">Outgoing Items Report</h4>
        ...
        $(document).ready(function(){
            $(".dropdown-item").on("click",function(){
                ...
                if ($(this).text() == "Date"){
                    ...
                    $('.find-content').remove();
                    $.ajax({
                        url : "{{route('inventory.outgoing-item.alldata')}}",
                        method : "POST",
                ...
                $(".card-plain").on("click",'button.find-content',function(){
                    ...
                    if (find_text == "Date"){
                        $.ajax({
                            url : "{{route('inventory.outgoing-item.date')}}",
                            method :"POST",
                            data : {"_token" : "{{csrf_token()}}",
                        ...
                        $.ajax({
                            url : "{{route('inventory.outgoing-item.time')}}",
                            method : "POST",
                            data : {"_token" : "{{csrf_token()}}",
                        ...
                        $.ajax({
                            url : "{{route('inventory.outgoing-item.datetime')}}",
                            method : "POST",
                            data : {"_token" : "{{csrf_token()}}",
                                "from_date" : $('div.find-content').children('input[type=date]').eq(0).val(),
                                "to_date" : $('div.find-content').children('input[type=date]').eq(1).val(),
